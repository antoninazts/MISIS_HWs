...
sum1 = n * (n + 1) / 2
# todo Тут из-за '/' на выходе получается float всегда с нулевой мантиссой так как n == int
...
n = float(input("n = "))
x = -b / a
if x >= m and x <= n: #todo pycharm предупреждает, оптимальнее так: m <= x <= n
...
for i in range(len(a)):
    b = int(a[i]) #todo при отрицательном числе тут сломается на минусе. Ну и по условию только "двузначное и трехзначное", поэтому решение для универсальной длины усложняет поставленную задачу, строки требуют больше памяти
sum7 += b
...
a = list(input("Государствa: ").split(" "))#todo если гос-во из нескольких слов, то сломается. По условию больше одного гос-ва не требуется.
...
born_in_winter_or_summer = bool(re.search(r"0[67812]|[12]", *month))#todo тут что-то не то, 10,11 месяц к примеру true, когда это false
...
#todo В общем все ок. Где заметил, оставил обратную связь
